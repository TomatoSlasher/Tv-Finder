...
const descriptionShow = document.querySelector(".show-desc");
...
// const getQuery = () => {
// console.log(searchForm.querySelector(".search-bar").value);
// };
// const addHandlerSearch = () => {
// searchForm.addEventListener("submit", function (e) {
// e.preventDefault();
// const search = searchBar.value;
// console.log(search);
// controlSearchResaults(search);
// if (search) {
...
const showTv = async function (show1) {
  try {
    ...
    // const tv = model.state;
    // rendiring tv data
    // console.log(tv.bg);
    const markup = `
        <img class="bg-image" src="${tv.bg}" alt="" />
        <div class="bottom">
          <img class="poster-image" src="${tv.medImage}" alt="" />
          <div class="show-text">
            <div class="h1-rating">
              <h1>${tv.title}</h1>
              <p class="rating">${tv.rating.average} <i class="fas fa-star"></i></p>
            </div>
            <div class="desc">
              <h1>Overview:</h1>
              <p class="description">
...
const controlSearchResaults = async function (name) {
  try {
    ...
    // const results = `
    // <li class="show-box">
    // <a href="${dataShort.id}"><img src="${dataShort.medImage}" alt="" /></a>
    // <p>${dataShort.title}</p>
    // </li>
    // `;
    const data1 = dataShort.map(x => `
      <li class="show-box">
                <a href=s"${x.id}"><img src="${x.orgImage}" alt="" /></a>
                <p>${x.title}</p>
              </li>
      `).join("");
    resultShow.insertAdjacentHTML("afterbegin", data1);
  } catch (err) {
    console.log(err);
  }
};
controlSearchResaults("dasd");
...
_parcelHelpers.export(exports, "live", function () {
  return live;
});
...
const loadTv = async function (show) {
  try {
    const res = await fetch(`http://api.tvmaze.com/shows/206`);
    ...
    // console.log(images);
    const bgImages = images.filter(i => i.type == "background");
...
const loadSearchResaults = async function (query) {
  try {
    ...
    // src.image.medium == null || undefined ? [""] : src.image.medium
    // );
    const cleanData = data.filter(src => {
      if (!src.show.image) {
        return;
...
},{}]},["1yWRd","BvQis"], "BvQis", "parcelRequireef1d")

//# sourceMappingURL=find.2aeafb3f.js.map
